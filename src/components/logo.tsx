...
function Logo({ size, className, ...props }: Props) {
  const getSize = () => {
    switch (size) {
      case 'sm':
        return { height: 24, width: 28, fontSize: 'text-sm/[12px]' };
      case 'md':
        return { height: 28, width: 32, fontSize: 'text-base/[14px]' };
      case 'lg':
        return { height: 32, width: 36, fontSize: 'text-lg/[16px]' };
    }
  };
  ...
  return (
    <div className={cn('inline-flex items-center gap-2', className)} {...props}>
      ...
      <span
        className={cn(
          'flex flex-col text-left font-extrabold text-sky-900',
          fontSize,
        )}
      >
        <span>Antarctic</span>
